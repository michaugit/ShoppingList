...
describe('ListItemComponent', () => {
  ...
  it('#integration successful delete item', fakeAsync(() => {
    const expectedDataResponse = {
      "message": "Pomyślnie usunięto przedmiot."
    }

    const itemService = fixture.debugElement.injector.get(ItemService)
    const spyListDelete = spyOn(itemService, 'delete').and.callThrough()
    const spyRemoveEmit = spyOn(component.removeItem, 'emit')
    ...
    const request = httpTestingController.expectOne('http://localhost:8080/api/item/'+ 'delete/' + component.item.id);
  ...
  it('#integration error delete item', fakeAsync(() => {
